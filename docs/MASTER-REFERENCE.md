...
| 044 | Cognee lädt .env CWD-abhängig (2026-04-26): Cognee 1.0.3 liest seine .env-Datei nur aus dem aktuellen Working-Directory. Pipeline-Skripte (ingest_to_cognee.py, cognee_search.py) lösen das durch eigenes .env-Loading via pipeline/_cognee_env.py BEVOR cognee importiert wird. Heißt: Skripte funktionieren von jedem Verzeichnis aus. Konsequenz für später: MCP-Server (Schritt 7) und Whisper-Pipeline müssen dasselbe Pattern nutzen. Test: cognee_search.py aus / aufgerufen findet die DBs korrekt. | Akzeptiert (Workaround) |

...
- Schritt 8: ✅ **DEPLOYED + verifiziert mit echten Daten (2026-05-02)**. Daily-Log Text-MVP end-to-end funktional: Telegram-Text → Jarvis-Bereinigung → memo_to_md → Cognee-Ingest → Knowledge-Graph-Abfrage. Drei bekannte Bugs offen (siehe Sektion 13a + ADR-050 Update): Bug 1 Kuzu-Lock-Konflikt (persistent, Fix=D3-Subprocess-Refactor identifiziert), Bug 2 Bot-MCP-Init nach Cognee-Restart (möglicherweise transient), Bug 3 Notion-Patient-Tool-Routing-Verwechslung (Datenleck-Risiko). 163 Tests grün (48 memo + 20 server + 73 hints + 15 ingest + 7 system_prompt). Voice (Schritt 8b) folgt nach Text-Verifikation.

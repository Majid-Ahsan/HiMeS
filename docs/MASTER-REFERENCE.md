...
- Schritt 8: 🟡 in Arbeit — End-to-End-Test (Voice-Memo abends → morgens Frage stellen → korrekte Antwort). Am 2026-04-27 **partiell** verifiziert (manueller Daily-Log → Cognee → Search → LLM-Antwort über Telegram), volle Voice→Whisper→Pipeline→Cognee-Kette über Telegram-Voice-Eingabe noch ausstehend. Text-MVP via ADR-050. Voice (Schritt 8b) folgt nach Text-Verifikation. voice_to_md.py wird zu memo_to_md.py refactored.

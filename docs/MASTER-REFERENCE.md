...
> **Version:** v25.2 · **Stand:** 2026-04-19 · **Pfad:** `docs/MASTER-REFERENCE.md`
...
| 1.5.25 | Wochentag-Halluzinations-Schutz | ✅ | Commit `068837f` (2026-04-19). System-Prompt-Regel "Wochentag-Regel (KRITISCH)" top-level eingefügt. HallucinationGuard-Domain `weekday` mit 2 Patterns (Wochentag+Datum within 40 chars, beide Orientierungen), Tool-Prefix zunächst `mcp__time__`, erweitert in 1.5.30 um himes-tools Datum-Tools. 12 neue Tests. |
...
| 1.5.32 | Calendar Assertion-Layer | ⬜ | Validator der Kalender-Antworten gegen Tool-Outputs prüft: jeder im Text genannte Wochentag+Datum-Paar muss durch Tool-Call belegt sein, jedes genannte Event muss im Tool-Output vorkommen (verhindert Event-zu-Tag-Drift wie 2026-04-19 Wochenübersicht). Soft-Guard-Design (Warning + Disclaimer, kein Rewrite). Nötig für langtext-Kalender-Formatierung, die in 1.5.30 nur teilweise durch Prompt-Regel adressiert wurde. Nach 1.5.26 einsortieren. |
...
- Event-zu-Tag-Bucketing: **nicht vollständig behoben** — Problem C tritt bei sehr langen Wochenübersichten weiter auf. Prompt-Regel reduziert Frequenz, ersetzt aber keine echte Validierung. **Phase 1.5.32** (Calendar Assertion-Layer, ⬜) ist die saubere Lösung und wird als offener Task dokumentiert.
...
- **Phase 1.5.32** Calendar Assertion-Layer: bei Wochenübersichten prüft Validator jede Event-zu-Tag-Zuordnung gegen Tool-Output. Soft-Guard mit Disclaimer bei Drift.
- **caldav-mcp Remote**: weiterhin unpushbar (upstream-Fork `madbonez/caldav-mcp`). 5 Commits auf VPS aus dieser Session (eddfab6, 1870046, plus 3 aus 1.5.21). Fork-Entscheidung offen.

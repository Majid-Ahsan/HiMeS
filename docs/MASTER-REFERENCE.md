...
Wenn `log_daily_entry` selbst fehlschlägt (OSError, FileExistsError nach Race-Condition), enthält der System-Prompt aktuell keine spezifische Anweisung — Jarvis reicht den `user_message_hint` per ADR-018-Konvention durch. Ausreichend für MVP. Bei häufigem Auftreten (unwahrscheinlich): Workflow-Block erweitern um „bei FileExistsError → retry mit `mode='replace'` nach User-Bestätigung".

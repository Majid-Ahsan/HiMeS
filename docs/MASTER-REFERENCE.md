...
> **Version:** v25.17 · **Stand:** 2026-05-03 · **Pfad:** `docs/MASTER-REFERENCE.md`
...
- Schritt 8: ✅ **FERTIG mit Bug-1-Fix (2026-05-03)**. Pipeline End-to-End mit echten Daten verifiziert (2026-04-21-Eintrag erfolgreich gespeichert + ingestet + via cognee_search wieder gefunden ohne Doppel-Restart). Bug 1 (Kuzu-Lock-Konflikt) strukturell behoben durch PR #14 (D3 Subprocess-Refactor, commit 0480fe1). Plus **Voice-Pipeline komplett implementiert** (PR #16-#19, ebenfalls 2026-05-03): Whisper-Marker `[🎤 Voice-Transkript]:` für LLM-Erkennung, Post-Processing-Garantie für Transkript-Echo (`input/voice_post_process.py`), deutsche Whisper-Konfig mit Eigennamen-Hint und ENV-Var-Konfigurierbarkeit (`input/whisper_config.py`). Verbleibende Bugs: Bug 2 (Bot-MCP-Init nach Cognee-Restart, weitgehend mitigiert durch D3), Bug 3 (Notion-Patient-Routing, Datenleck-Risiko, separater PR geplant) und Bug 4 (Cognee-LLM-Halluzination beim Graph-Build, eigene Phase wert). 163 Daily-Log-Tests weiterhin grün (48 memo + 20 server + 73 hints + 15 ingest + 7 system_prompt). Lokale Voice-Test-Suite: 109 passed (+5 neu für whisper_config). Voice (Schritt 8b) als End-to-End-Pipeline verifiziert.
...
| 046 | Bot Async-Generator-Deadlock (2026-04-25/26): HiMeS-Bot deadlocked nach 25.04.2026 17:50 — keine Telegram-Antworten mehr für ~14h. Logs zeigen async-Generator-Cleanup-Race: „aclose(): asynchronous generator is already running" plus „Cancelled via cancel scope ... in different task than entered". Self-Healing erst durch Container-Restart am 26.04.2026 08:23. Wichtige Beobachtung: CalDAV-MCP-Service selbst war die ganze Zeit gesund (lokal und extern via curl bestätigt — HTTP 200 auf beiden), Problem lag im SDK-Client / mcp-remote-Client im Bot. Ursache vermutlich Race im `SubprocessCLITransport._read_messages_impl` oder in der `SDKClient`-Singleton-Restart-Logik (siehe ADR-024 SDK-Limit, OPS-NOTES SDK-Usage). Workaround: Container-Restart heilt es. Mögliche Lösungen für später: Self-Healing-Logik (Bot erkennt Deadlock und restartet sich), bessere async-Cleanup-Pattern, oder Upgrade auf neuere claude-code-sdk-Version falls Bug dort gefixt. **Update 2026-04-27**: Warnung sporadisch erneut beobachtet beim Bot-Restart in Phase 2.1 Schritt 7, kein Deadlock entstanden — bestätigt sporadische Natur, nicht-blockierend. **Update 2026-04-30 — zwei distinkte Symptome beobachtet**: (A) **Lautes Crashing** am 28.04.2026 ~22:19 UTC bei `sdk_client.day_changed` ausgelöst, voller Stack-Trace mit `Attempted to exit cancel scope in a different task` plus `RuntimeError: aclose(): asynchronous generator is already running`. Container wurde durch `Restart=always` automatisch neu gestartet, danach stundenlang stabil (Cognee 5× ohne Probleme abgefragt) — Self-Recovery funktioniert für diesen Fall. (B) **Lautloser Hänger** am 28.04.2026 um 17:15 UTC: nach `orchestrator.message_received` keine weitere Log-Zeile für 4 Stunden, kein Stack-Trace, kein Error, systemd merkte nichts (Container lebt aber antwortet nicht). Manueller `docker compose restart` heilte. Symptom B ist das schlimmere — bestätigt explizit was zur Restart-Policy schon stand: `Restart=always` greift NICHT bei stillen Deadlocks ohne Crash. Lösungsoptionen für später: (1) HTTP-Watchdog auf einen Liveness-Endpoint, der bei mehr als N Sekunden ohne Heartbeat den Container forciert restartet; (2) Upgrade auf neuere claude-code-sdk-Version falls Bug dort gefixt; (3) Self-Healing-Logik im Bot (Deadlock-Detection und Selbstrestart). Kurzfristig: User hat `docker compose restart` als Alias `jarvis-restart` parat (siehe Sektion 13a Aufräum-Session-Eintrag). | Bug bekannt, sporadisch |
...
| 050 | Daily-Log Text-Pipeline (MVP, Phase 2.1 Schritt 8, 2026-05-01): Eigener `daily-log`-MCP nach T1-Pattern (jarvis-daily-log.service auf Port 8003, Caddy-Proxy `daily-log-ahsan.duckdns.org`, ADR-047). `pipeline/voice_to_md.py` wird zu `pipeline/memo_to_md.py` refactored: Frontmatter um `tags`/`entities` erweitert, Datums-Anker-Zeile als erste Body-Zeile, Append-Logik inkl. `## (Erster Eintrag)`-Wrap entfernt, statt dessen zwei Modi `write`/`replace`. Mehrere Memos pro Tag werden vom Bot via Read+LLM-Merge zu kohärentem Fließtext verschmolzen (D3) statt `## HH:MM`-Append-Stack — passt zu Schema-Beispielen 2026-04-13/14/15. MCP importiert `pipeline.memo_to_md` und `pipeline.ingest_to_cognee` direkt (Direkt-Import statt Subprocess, D4) — kein Stdout-Parsing. Text-Bereinigung + Tags/Entities-Extraktion im Bot (D5), Preview+Bestätigung Pflicht (D6), Hints deterministisch im MCP, Vorschlags-Formulierung im Bot (D7), Cognee-Ingest async via `asyncio.create_task` mit Sofort-Bestätigung (D8). Tool-Set (D9): `log_daily_entry(text, user, date, tags, entities, mode)` + `read_daily_log(date, user)`. Voice (Schritt 8b) folgt nach Text-Verifikation und braucht nur Whisper als Vor-Stufe vor `memo_to_md`. Volle Begründungen, Trade-offs, Nicht-Ziele und akzeptierte Schuld in `docs/adr/050-daily-log-text-pipeline.md` (Format-Abweichung von ADR-047/048/049 wegen 9 strukturierter Sub-Entscheidungen). | Akzeptiert |
...
**voice_to_md→memo_to_md Refactor (ADR-050 D2)** (Phase 2.1 Schritt 8, 2026-05-01)
Append-Logik inkl. `## (Erster Eintrag)`-Sonderfall wird entfernt; Subset der 16 Tests aus Phase 2.1 Schritt 5 wird obsolet/muss umgeschrieben werden. Neue Modi `write` (Default, fail bei existierender Datei) und `replace` (überschreibt komplett) ersetzen die bisherige `## HH:MM`-Append-Strategie. Mehrere Memos pro Tag werden stattdessen vom Bot via Read+LLM-Merge zu kohärentem Fließtext verschmolzen (siehe ADR-050 D3). Begleit-Änderungen am Skript: Frontmatter um `tags`/`entities` erweitert, Datums-Anker-Zeile als erste Body-Zeile.

**`memo_to_md.write_memo()` polymorphe Argument-Signatur** (Phase 2.1 Schritt 8, 2026-05-01)
`tags` und `entities` akzeptieren sowohl `list[str]` als auch Komma-String. Praktisch fürs MCP, das LLM-Output verarbeitet, aber Polymorphismus versteckt Erwartungen. Aktuelle MCP-Aufrufe übergeben immer Listen, String-Pfad ist tot. Kein User-sichtbares Problem, aber Refactor-Kandidat für späteren Konsolidierungs-Sweep.

**`log_daily_entry`-Selbst-Failure ohne System-Prompt-Branch** (Phase 2.1 Schritt 8, 2026-05-01)
Wenn `log_daily_entry` selbst fehlschlägt (OSError, FileExistsError nach Race-Condition), enthält der System-Prompt aktuell keine spezifische Anweisung — Jarvis reicht den `user_message_hint` per ADR-018-Konvention durch. Ausreichend für MVP. Bei häufigem Auftreten (unwahrscheinlich): Workflow-Block erweitern um „bei FileExistsError → retry mit `mode='replace'` nach User-Bestätigung". Race-Window: Zwischen Schritt 0 (`read_daily_log` → `exists=false`) und Schritt 5 (`log_daily_entry mode=write`) liegt ein theoretisches Race-Fenster. Im Single-User-MVP irrelevant (Bot-Turns sequentiell, LLM single-threaded pro Turn). Wird relevant ab Phase 2.10 (Multi-User) oder Phase 2.6 (Sub-Agents): dann muss `log_daily_entry` entweder file-locking nutzen oder der Workflow muss FileExistsError explizit als Auto-Retry-mit-replace nach User-Bestätigung handhaben.

**Cognee-Kuzu-Lock-Konflikt zwischen daily-log-MCP und cognee-MCP Service** (Phase 2.1 Schritt 8, 2026-05-02)
Beide Cognee-Process-Instanzen (jarvis-cognee.service und jarvis-daily-log.service) halten überlappende Write-Locks auf den Kuzu-Graph-DB-Files. Cognee 1.0.3 hat keine Connection-Close-API. Sobald daily-log-MCP einen Ingest macht, blockiert er Cognee-Search bis Doppel-Restart aller Services. ADR-050 D4 (Direct-Import von process_files) ist mit Single-Process-Kuzu strukturell unvereinbar. Fix-Strategie identifiziert: D3 = Subprocess-basierter Ingest in daily-log-mcp/ingest.py via asyncio.create_subprocess_exec. Subprocess endet nach Ingest, Lock automatisch frei. ~2h Implementierung. Zwischenzeitlicher Workaround: manuelles Doppel-Restart (daily-log → cognee → Bot) nach Bedarf.
**Update 2026-05-03:** Behoben durch PR #14 (D3-Subprocess-Refactor). Bot-Restart + Telegram-Test mit echtem 21.04.2026-Eintrag verifiziert End-to-End: Speichern + Ingest + cognee_search funktionieren ohne Doppel-Restart. Verifikations-Beweis: Subprocess-Logs zeigen "Ingest done" um 11:54:29, Cognee-Search ~30-60s später findet den Eintrag mit allen Personen (Mariette, Newsha, Fateme, Großmutter, Vater) und thematischen Querverbindungen zu anderen April-Einträgen.

**Bot-SDK-Client Init-Failure nach Cognee-Service-Restart** (Phase 2.1 Schritt 8, 2026-05-01)
Nach Cognee-Service-Restart führt der Bot-SDK-Client manchmal alte Session-ID weiter. Cognee-MCP-Server lehnt mit "request before initialization complete" ab. Jarvis paraphrasiert das als "Parameterfehler". Workaround: Bot-Container nach jedem Cognee-Restart auch restarten. Am 2026-05-02 nicht reproduziert — möglicherweise transient. Ggf. mit D3-Fix (Bug 1) automatisch behoben, da dann kein Cognee-Restart mehr nötig.
**Update 2026-05-03:** Mit D3-Fix (Bug 1) ist kein Cognee-Restart mehr nötig nach Daily-Log-Ingest. Damit ist der Trigger für Bug 2 weitgehend eliminiert. Falls Cognee-Restart aus anderen Gründen nötig wird (z.B. Service-Update), kann Bug 2 noch auftreten — bleibt als Schuld dokumentiert, aber Priorität deutlich gesunken.

**Notion-Patient-Verwechslung mit privaten Personennamen** (Phase 2.1 Schritt 8, 2026-05-02)
Auf Frage "was weisst du über neda?" hat der Bot zusätzlich zu Cognee/Daily-Log auch Notion durchsucht und Patientendaten der Patientin "Neda Naghavi" ausgegeben — verwechselt mit Majids Frau Neda. **Datenleck-Risiko: ärztliche Schweigepflicht-Verletzung.** Fix: System-Prompt-Erweiterung — Tool-Routing-Disziplin-Block der explizit definiert: Persönliche Personenfragen → Cognee/Daily-Log primär. Notion-Patientensuche nur bei explizitem medizinischem Kontext oder Patient-IDs. Bei Mehrdeutigkeit: nachfragen, nicht raten. ~30min Implementierung.

**Cognee-LLM-Halluzination beim Knowledge-Graph-Build** (Phase 2.1 Schritt 8, 2026-05-03)
Cognee 1.0.3 nutzt Claude Sonnet 4 sowohl beim Ingest (Beziehungs-Extraktion via @cognee.agent) als auch beim Search (Result-Synthese). LLM-Inferenz zieht manchmal falsche Verknüpfungen zwischen räumlich-zeitlich nahen Knoten. Beobachtetes Beispiel: Im 2026-04-21-Eintrag stand wörtlich "Mariette krankgemeldet" und in einem späteren Absatz "ich hatte Herzkatheterlabor-Spätdienst". Cognee-Search-Antwort hat daraus "Mariette arbeitet im Herzkatheterlabor" gemacht — plausibel klingend, aber nicht im Original. Risiko-Profil: bei privaten Tagebuch-Daten kosmetisch ärgerlich. Bei medizinischen Patientendaten potenziell gefährlich (falsche Diagnose-Verknüpfungen, falsche Medikamenten-Zuordnungen). Mitigation für jetzt: Bei wichtigen Cognee-Antworten gegen Original-Eintrag verifizieren. Strukturelle Lösungen (z.B. Konfidenz-Schwellen für extrahierte Beziehungen, Source-Citation in cognee_search-Output, Entity-Boundary-Rules im Cognee-Prompt) sind eigene Phase wert. Verwandt mit Bug 3 (Notion-Patient-Routing) — beide sind LLM-Inferenz-Fehler bei Datenausgabe.

**ADR-046 Bug B — zweite dokumentierte Manifestation** (Phase 2.1 Schritt 8, 2026-05-01 22:01:28 UTC)
Tageswechsel-Trigger bei UTC-Mitternacht (= Berlin 00:01 lokale Zeit) während aktivem Daily-Log-Workflow ausgelöst. Stack-Trace matcht Symptom A genau. Auto-Recovery via restart: unless-stopped funktionierte wie geplant. Conversation-State verloren (in-memory only). User-Erfahrung: Bot fragte "Hallo! Wie kann ich dir helfen?" mid-workflow. Validiert ADR-046 — Bug B ist trigger-getrieben, nicht uptime-getrieben.

**Bot-Code lebt im Docker-Image, nicht als Volume-Mount** (Phase 2.1 Schritt 8, 2026-05-01, Schritt 9g)
docker-compose.yml mountet nur ./data, ./logs, ./config als Volumes. Code-Verzeichnisse (core/, pipeline/, daily-log-mcp/) sind IM Image gebaut. Konsequenzen für Deploy: mcp_config.json-Änderungen → docker compose restart himes reicht. core/sdk_client.py / core/claude_subprocess.py / Code-Module: docker compose up -d --build himes notwendig (Rebuild ~30-60s). Sollte in Future-Deploy-Doku beachtet werden.

**Cognee-Failure-Graceful-Fallback funktioniert** (Phase 2.1 Schritt 8, 2026-05-02, Beobachtung)
Bei Cognee-Search-Failure (Bug 1) hat Jarvis korrekt graceful zurückgefallen auf andere Tools (read_daily_log, CalDAV) und eine sinnvolle Antwort aus den verfügbaren Quellen synthetisiert. Failure-Hint vom Cognee-Tool wurde transparent kommuniziert. ADR-018-Format-Konvention bewährt sich in Multi-Tool-Setups.

**Voice-Pipeline für Telegram-Audio-Inputs** (Phase 2.1, 2026-05-03)
Vier PRs implementieren End-to-End-Voice-Workflow auf einer bereits funktionalen Whisper-Transkription:

**PR #16** (`feat/voice-prompt-improvements`, commit 9d031e3) — Voice-Marker `[🎤 Voice-Transkript]:` als Prefix für Whisper-Transkripte in `input/telegram_adapter.py:_handle_voice`, damit Bot-LLM Voice-Inputs erkennt. Plus System-Prompt-Block mit Sprach-Hint (99.99% Deutsch, alternativ Farsi/Englisch) und Format-Anweisung für Transkript-Echo (kursive Vorzeile `_„...“_`).

**PR #17** (`fix/voice-block-stronger`, commit 654f5b1) — Voice-Block prominent ans Anfang des System-Prompts verschoben (Zeile 426 → 49) + direktive Formulierung („KRITISCHE REGEL", „MUSS", „VORRANG"). Funktionierte in 1-Turn-Antworten, wurde aber in Multi-Tool-Workflows (Wetter, Bahn) zuverlässig ignoriert — bekanntes LLM-Verhalten in komplexen Tool-Sessions.

**PR #18** (`feat/voice-transcript-post-process`, commit 7a82db1) — Lösung: Post-Processing im Telegram-Adapter. Pure Helper-Modul `input/voice_post_process.py` (analog zu PR #14 Subprocess-Helper-Pattern, testbar ohne Telegram-Stack). Bei Voice-Input wird nach LLM-Response geprüft ob Antwort mit `_` beginnt, sonst manuell `_„<transcript>“_\n\n` vorangestellt. Garantiert konsistentes Voice-Format unabhängig von Tool-Anzahl. System-Prompt-Block dadurch wieder vereinfachbar (von KRITISCHER REGEL auf Hint-Level, -1081 chars: 20673 → 19592). Edge-Case-Tests: bereits vorhandenes `_`-Echo wird nicht doppelt eingefügt, leading whitespace wird via lstrip erkannt.

**PR #19** (`feat/whisper-language-config`, commit 856c21a) — Whisper-Konfiguration: `language="de"` als Default (eliminiert ~80% Auto-Detect-Fehler — beobachtet vorher: „Wie ist das Wetter heute in Mülheim" → „Wies, das fährt heute in Mulheim", auch chinesische Chars `察` in deutschen Audios), `initial_prompt` mit Familien- und Tech-Eigennamen (Majid, Neda, Mariette, Mülheim, Echolabor, Cognee, Jarvis, ...) für bessere Eigennamen-Erkennung, ENV-Vars `WHISPER_MODEL` / `WHISPER_LANGUAGE` / `WHISPER_INITIAL_PROMPT` für späteres Modell-Upgrade ohne Code-Refactor. Eigenes Light-Modul `input/whisper_config.py` analog zu Post-Process-Helper. Implementation via `functools.partial` damit `loop.run_in_executor` die kwargs durchreicht.

Verifikation 2026-05-03: VPS-Deploy auf Merge-Commit 3080922, Container-Rebuild erfolgreich, `whisper_config`-Import zeigt `language=de, model=base, prompt-snippet: Majid, Neda, Taha, ...` korrekt geladen. Pytest gesamt: 109 passed (+5 neu für whisper_config), 0 Regressions.

Restschuld: Whisper-`base`-Modell hat noch gelegentliche Hörfehler bei kurzen/rauschigen Audios. Stufe-2-Upgrade auf `small` (466 MB, deutlich bessere DE-Qualität) möglich via `WHISPER_MODEL=small` ENV-Var, braucht aber VPS-RAM-Erweiterung (aktuell 3.7 GB total / 1.6 GB available — `small` würde knapp). `faster-whisper` als 2-4× schnellerer Replacement existiert, ist aber größerer Refactor — nur sinnvoll wenn nach Stufe-2 die CPU-Latenz zu hoch wird.

**Aufräum-Session 2026-04-30 — sechs Wartungs-Punkte erledigt** (Phase G+, 2026-04-30)
Sechs kleinere Wartungsmaßnahmen auf VPS nach Phase-7-Abschluss, dokumentiert hier statt in eigener Phase. (1) **Alte Cognee-UUID-Verzeichnisse aufgeräumt**: `137c38f4-...`, `26b6f2ad-...`, `3a819802-...`, `5a6295e4-...` plus die globale `cognee_graph_kuzu`-Datei (Reste aus früheren Prune-Zyklen vor Phase 6c-data Round 2) gelöscht; aktive User-DB `3b0d114d-...` und `cognee_db` SQLite-Metadata erhalten. lsof-Verifikation zeigt nur noch Filehandles auf der aktiven DB; `jarvis-cognee.service` HTTP 200 nach Cleanup. (2) **Verwaistes daily-logs-Verzeichnis** `/home/ali/cognee/data/daily-logs/` entfernt — Rest aus früher Test-Phase vor `--data-dir`-Standardisierung; korrekter Pfad ist `/home/ali/himes-data/memory/daily-logs/`. (3) **HIMES_DATA_DIR-Env-Var** in `~/.bashrc` gesetzt (`HIMES_DATA_DIR=/home/ali/himes-data`) — Schritt-6-Ingest-Skript braucht jetzt kein `--data-dir`-Argument mehr; live verifiziert: `ingest_to_cognee.py --all --yes` findet alle 5 Daily-Logs und meldet alle als „skip (unveraendert, hash matches)" → Re-Ingest-Schutz arbeitet wie erwartet. (4) **Fünf Bash-Aliases** in `~/.bashrc`: `dcl` (`docker compose logs`), `jarvis-restart`, `jarvis-logs`, `cognee-restart`, `cognee-ingest`; live verifiziert via `dcl ps`. (5) **NOPASSWD-Sudoers** in `/etc/sudoers.d/jarvis-services` mit sechs eng eingeschränkten Befehlen: `ali ALL=(ALL) NOPASSWD: /usr/bin/systemctl {start,stop,restart} jarvis-cognee.service` plus dieselben drei für `jarvis-caldav.service` — kein Pauschal-NOPASSWD. Live verifiziert: `sudo systemctl restart jarvis-cognee.service` ohne Passwort-Prompt, Service danach HTTP 200. Adressiert die Schmerzphase aus Phase 6c-data Round 2, in der ein Service-Stop für `cognee.prune.prune_system()` manuell durch User mit Passwort durchgeführt werden musste. (6) **Bug-B-Beobachtungen** vom 28.04.2026 (Symptom A „lautes Crashing" mit Auto-Restart vs. Symptom B „lautloser Hänger" 4h ohne Log) — vollständige Analyse direkt in ADR-046 (Sektion 13) angehängt. Diese Mini-Phase ist nicht Phase 7, sondern Aufräum-Session danach (Phase-G+-Bezeichnung intern), Doku-only Commit `docs/cleanup-2026-04-30`.
...
| Datum | v | Änderung |
|---|---|---|
| 2026-05-03 | 25.17 | **Phase 2.1 Voice-Pipeline End-to-End implementiert** (Code + Doku). Vier PRs am selben Tag: (a) **PR #16** Voice-Marker `[🎤 Voice-Transkript]:` + System-Prompt-Sprach-Hint (99.99% Deutsch, alternativ Farsi/Englisch) + Format-Anweisung für Transkript-Echo. (b) **PR #17** Voice-Block ans Anfang des Prompts verschoben + direktive Formulierung — funktionierte in 1-Turn-Antworten, wurde aber in Multi-Tool-Workflows ignoriert. (c) **PR #18** Post-Processing-Garantie via Pure-Helper-Modul `input/voice_post_process.py` — analog zu PR #14-Subprocess-Pattern, testbar ohne Telegram-Stack. System-Prompt-Block dadurch wieder vereinfacht (von KRITISCHER REGEL auf Hint-Level, -1081 chars: 20673 → 19592). (d) **PR #19** Whisper-Konfig: `language="de"` als Default + `initial_prompt` mit Familien- und Tech-Eigennamen + ENV-Vars `WHISPER_MODEL` / `WHISPER_LANGUAGE` / `WHISPER_INITIAL_PROMPT` in eigenem Light-Modul `input/whisper_config.py` für späteres Modell-Upgrade ohne Code-Refactor. Pytest gesamt: 109 passed (+5 neu), 0 Regressions. VPS-Deploy auf 3080922 verifiziert: `language=de, model=base, prompt-snippet: Majid, Neda, Taha, ...` korrekt im Container. Sektion 13a um Voice-Pipeline-Eintrag erweitert mit Restschuld-Notiz (Stufe-2 = `small`-Modell braucht VPS-RAM-Erweiterung). |
| 2026-05-03 | 25.16 | **Phase 2.1 Schritt 8 final fertig — Bug 1 behoben durch D3-Refactor** (Doku-Update, kein Code). (a) ADR-050 D3 implementiert via PR #14 (commit 0480fe1) — Subprocess-basierter Ingest in `daily-log-mcp/ingest.py` ersetzt Direct-Import von `process_files`. (b) End-to-End-Verifikation mit echtem 2026-04-21-Tagebuch-Eintrag (~2580 Zeichen): Speichern + Ingest + cognee_search ohne Doppel-Restart. (c) Schritt-8-Status auf „✅ FERTIG mit Bug-1-Fix (2026-05-03)" aktualisiert. (d) Sektion 13a Bug-1 + Bug-2 mit Update-Blöcken vom 2026-05-03 ergänzt — Bug 1 behoben, Bug 2 Trigger weitgehend eliminiert. (e) ADR-050 neuer Update-Block „D3 implementiert + verifiziert" mit Verifikations-Beweis. (f) Neuer Bug-4-Eintrag in Sektion 13a: Cognee-LLM-Halluzination beim Graph-Build (Mariette-Beobachtung 2026-05-03) — Phase-Status um Bug-4-Referenz erweitert, ADR-050 D3-Block mit Halluzinations-Hinweis ergänzt. |
| 2026-05-02 | 25.15 | **Phase 2.1 Schritt 8 — DEPLOYED + verifiziert** (Status-Update, kein Code). (a) Schritt-8-Status von „🟡 in Arbeit" auf „✅ DEPLOYED + verifiziert mit echten Daten (2026-05-02)" aktualisiert, 163 Tests grün (48 memo + 20 server + 73 hints + 15 ingest + 7 system_prompt). (b) Sechs neue Einträge in **Sektion 13a**: Bug 1 Kuzu-Lock-Konflikt (Two-Process-Architektur, D4→D3-Revert identifiziert), Bug 2 Bot-SDK-Client Init-Failure nach Restart (möglicherweise transient), Bug 3 Notion-Patient-Verwechslung (Datenleck-Risiko, Schweigepflicht), ADR-046 Bug B zweite Manifestation (Tageswechsel-Trigger validiert), Bot-Image vs Volume-Mount Deploy-Konvention, Cognee-Failure-Graceful-Fallback (positives Pattern). (c) **ADR-050 Update-Block**: D4 revidiert — Kuzu single-process-only via flock(2), D3 (Subprocess-Ingest) wird re-aktiviert. |
